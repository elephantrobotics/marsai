...
class ModelClassifier:
    ...
    def get_mode(self, sensor_input):
        ...
        mode, data = self.model_classifier(vision, voice, touch, distance)
        return mode, data

    def model_classifier(self, _vision, _voice, _touch, _distance):
        mode = ''
        data = []
        # update_index = 4
        ...
        #mode = self.update_times(update_index, mode)
    ...
    def get_sum(self, data_input):
        _sum = 0
        for i in data_input:
            _sum += i
        return _sum

    def update_times(self, update_index, mode):
        _mode = mode
